...
object PlayerHandler {
  ...
  final case class GetIPInfo(replyTo: ActorRef[GetIPInfoResponse]) extends Command
  final case class ServerBinding(ip: String, port: Int, replyTo: ActorRef[GetIPInfoResponse]) extends Command
  ...
  def apply()(implicit system: ActorSystem[GameServer.Command]): Behavior[Command] = {
    Behaviors.setup(ctx => {
      ...
      val serverBinding = connectionListener.toMat(Sink.foreach { connection =>
      ...
      Behaviors.receiveMessage {
        case GetIPInfo(replyTo) => {
          ctx.pipeToSelf(serverBinding) {
            case Success(binding) => ServerBinding(binding.localAddress.getHostString, binding.localAddress.getPort, replyTo)
